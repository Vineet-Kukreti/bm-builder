...
| **Installed Claude Code but the app still shows it as missing** | A running app keeps the PATH it started with, so a freshly-installed command can be invisible to it. Click **↻ Re-check** on the dashboard; if it still doesn't show, **restart the app** (`Ctrl+C`, then `.\run.ps1`). Confirm it's actually installed by running `claude --version` in a **new** PowerShell window. |
